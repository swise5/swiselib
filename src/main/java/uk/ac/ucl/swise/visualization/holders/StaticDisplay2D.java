...
package uk.ac.ucl.swise.visualization.holders;
...
/**
 * A dirty kludge of the Display2D class, repurposed for static ShapeFile visualisation
 * 
...
public class StaticDisplay2D extends JComponent implements Manipulating2D
    {
    protected boolean precise = false;
        
    public String DEFAULT_PREFERENCES_KEY = "StaticDisplay2D";
    ...
    public class OptionPane extends JFrame
        ...
        NumberTextField xOffsetField = new NumberTextField(0,1,50)
            ...
            public double newValue(final double val)
                ...
                StaticDisplay2D.this.repaint();  // redraw the inside display
        ...
        OptionPane(String title)
            ...
            useBuffer.setToolTipText("<html>When using transparency, <i>or</i> when on a Mac,<br>this method is usually faster, but may require more<br>memory"
            		+ " (especially on Windows/XWindows) --<br>increasing heap size can help performance.</html>");
...
/*                Preferences appPrefs = Prefs.getAppPreferences(simulation, getPreferencesKey());
    ...
    public class InnerDisplay2D extends JComponent
        ...
        public void paintToMovie(Graphics g)
          ...
          //      long steps = StaticDisplay2D.this.simulation.state.schedule.getSteps();
                if (//steps > lastEncodedSteps &&
                    shouldUpdate()
                    )// && StaticDisplay2D.this.simulation.state.schedule.getTime() < Schedule.AFTER_SIMULATION)
                    {
                    StaticDisplay2D.this.movieMaker.add(paint(g,true,false));
        ...
        public String getToolTipText(MouseEvent event)
            ...
            if (useTooltips)
                ...
                return createToolTipText( new Rectangle2D.Double(point.x,point.y,1,1));
                }
            else return null;
            }
...
//        createConsoleMenu();
    ...
    */
    public StaticDisplay2D(final double width, final double height, GUIState simulation, long interval)
        {
    ...
    public StaticDisplay2D(final double width, final double height, GUIState simulation)
        {
...
//        createConsoleMenu();
    ...
    */
    ...
    DrawInfo2D getDrawInfo2D(FieldPortrayal2DHolder holder, Rectangle2D clip)
        ...
        DrawInfo2D d2d = new DrawInfo2D(null, holder.portrayal, region, clip);
        d2d.gui = null;
        d2d.precise = precise;
        return d2d;
        }
    ...
    public void createInspectors( final Rectangle2D.Double rect)
        {
        Bag inspectors = new Bag();
        Bag names = new Bag();
