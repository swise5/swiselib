...
import swise.main.objects.network.GeoNetwork;
import swise.main.objects.network.GeoNode;
import swise.main.objects.network.ListEdge;
...
public class NetworkUtilities{
	 ...
	 */
	static GeoNode getNode(Coordinate c, Continuous2D field, GeomVectorField fields, 
			HashMap <MasonGeometry, GeoNode []> linkToNodeMapping, 
			HashMap <MasonGeometry, ListEdge> geomToEdgeMapping, GeoNetwork network, double resolution, GeometryFactory fa){
		Double2D loc = new Double2D(c.x, c.y);

		Bag b = field.getNeighborsWithinDistance(loc, resolution, false);
		if(b == null || b.size() == 0){
						
			GeoNode node = new GeoNode(fa.createPoint(c), network.getNextNodeIndex());
...
//		        		System.out.println("foundya");
	...
	public static void testNetworkForIssues(GeoNetwork n){
	 ...
	 * the road network consists only of a large, fully-connected component.
	 */
	public static GeoNetwork multipartNetworkCleanup(GeomVectorField roadLayer, Bag roadNodes, double resolution, 
		...
		GeoNetwork net = new GeoNetwork(false);
...
/*			if(testIndex % 100 == 0)
	...
	*/	}
	 ...
	 */
	static Bag getSubcomponents(GeoNetwork network, MersenneTwisterFast random){
...
//		int count = nodes.size() / 50;
		int count = (int) Math.max(3, Math.log(nodes.size()));
		...
		for(int i = 0; i < count; i++){
			...
			while(//openSet.size() > biggestSize && 
					openSet.size() > 0){
			...
			else if(found.size() < nodes.size() / 2.){
	 ...
	 */
	public static ArrayList <Edge> findLinkTo(GeoNetwork subgraph, GeoNetwork all, GeoNode target, AStar astar){
	 ...
	 */
	public static void attachUnconnectedComponents(GeoNetwork subgraph, GeoNetwork all){
...
//					all, allNodes, 1.);
	 ...
	 */
	public static ArrayList <Bag> connectedComponents(GeoNetwork graph){
		ArrayList <Bag> subcomponents = new ArrayList <Bag> ();
	...
	public static boolean validPath(ArrayList <Edge> path, GeoNetwork network){
